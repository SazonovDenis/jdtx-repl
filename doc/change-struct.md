## Методика и алгоритм смены структуры БД в репликационной сети
...
сервер получает ответы и отмечает у себя, что станции "замолчали". 

Приняв команду `mute` и ответив на нее `mute-done` станция гарантированно обработала ту очередь, 
которая была на сервере _в момент подачи_ команды `mute`, однако на станцию могут позже попасть
запоздалые реплили от других станций (отправленные другими станциями _после_ того, как _эта_ станция уже замолчала), 
и эти запоздалые реплики будут еще _старой_ структуры.

Убеждаемся, что все станции не только замолчали, но и обработали все запоздавшие чужие реплики. 
Для этого на сервере, после получения от всех станций ответа `mute-done` (замолчали все станции), запоминаем maxAge. 
После посылаем вторую команду `mute`, при ответе на нее фиксируем _новый_ возраст _входящей очереди_. 
Когда этот возраст на всех станциях, станет не менее ранее запомненного maxAge это будет означать, что необработанных реплик 
в системе больше нет и не будет.   
   
После того как сервером от всех станций получено уведомление, что они замолчали 
(а значит, что еще ранее станциями были отправлены последние реплики в старой структуре, и других не будет), 
после того как мы убедились, что все реплики всеми станциями приняты и обработаны,
можно начинать _фактическую смену_ структуры БД и ее _фиксацию_.

### Смена структуры БД 

##### Шаги по смене структуры по всей репликационной сети  
...
- Ожидание, что все рабочие станции замолчали и отчитались об этом. Запоминаем maxAge.
- Повторная отправка команды `mute-all` всем станциям.
- Станци принимают повторный сигнал, и снова отвечают `mute-done`. 
- Сервер, получив повторный сигнал `mute-done`, ожидает, когда что minAge по всем станциям станет больше ранее запомненного maxAge. 
