...
Если при старте репликатора реальная структура не совпадет с утвержденной, 
то репликатор просто останавливается (и ждет следующего запуска).

Если "реальная" структура не совпадет с "зафиксированной", то репликатор останавливатся.

Если на станцию со старой структурой БД попадают реплики от новой структуры БД, 
то применение таких реплик откладывается до момента совпадения структуры БД на этой станции со структурой БД реплики. 
...
При начальной инициализации репликации текущая "реальная" структура БД просто копируется как "утвержденная", 
